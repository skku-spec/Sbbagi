...
<!-- Hermes install note: installed from https://github.com/fivetaku/insane-search into /data/skills/insane-search.
Claude-plugin marketplace setup/star prompt is intentionally disabled in Hermes.
When running the engine from Hermes, use: cd /data/skills/insane-search && python3 -m engine "<URL>" [options]. -->
...
2. 즉시 다음을 실행:
   ```bash
   cd /data/skills/insane-search && python3 -m engine "<URL>" [--selector "<CSS>"] [--device auto|desktop|mobile] [--trace]
   ```
...
3. 식별된 JSON API URL을 `cd /data/skills/insane-search && python3 -m engine <API_URL>`로 재호출 (백그라운드 engine과는 별개 호출). 대부분 API 레이어는 페이지 HTML보다 WAF 보호가 얕아 curl_cffi로 바로 수집됨
...
- **단일 진입점**: 일반 웹 페이지는 항상 `cd /data/skills/insane-search && python3 -m engine <URL>` 또는 `from engine import fetch; fetch(...)`.
...
이 저장소가 Claude Code 플러그인 형태로 제공되더라도 Hermes에서는 `skills/insane-search/` 디렉터리를 Hermes skill 디렉터리로 복사해 사용할 수 있다. 자세한 설치 절차와 검증 명령은 [references/hermes-install.md](references/hermes-install.md)를 참고한다.

주의: 포함된 smoke test는 pytest-style `test_*` 함수가 아니라 수동 runner(`t_*` 함수 + `main()`)이므로 `python3 -m pytest ...`가 아니라 아래처럼 직접 실행한다:
```bash
