...
For `vibe-notion` specifically (agent acts as the logged-in user by reading the Notion desktop app session), see `references/vibe-notion-cloud-hosting.md`. In short: use a GUI Windows/macOS machine for personal-session mode; use `vibe-notionbot`, `ntn`, or direct API on headless servers.
